...
private:
   ...
   /* =================== private helper function declarations ======= */
    ...
    double weightedEntropy(stats& s, pair<int,int> ul, pair<int,int> lr);

    void renderHelper(PNG & img, Node* curr);

    void pruneHelper(twoDtree::Node* subtree, double tol);

    void calculateNewCoords(stats &stats, pair<int, int>& ul, pair<int, int>& lr, bool vert);

    void shouldBePruned(Node *pNode, HSLAPixel pixel, double tol, double& prunable, double& total);

    void clearAll(Node *subRoot);

    Node* copyAll(Node* tree);
